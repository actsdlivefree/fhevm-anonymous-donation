...
async function resolve(
  ...
  const _mockChains: Record<number, string> = {
    ...
    1337: "http://localhost:8545",  // Alternative hardhat chainId
    ...(mockChains ?? {}),
  };
...
export const createFhevmInstance = async (parameters: {
  ...
  // Resolve chainId
  const { isMock: resolvedIsMock, rpcUrl: resolvedRpcUrl, chainId } = await resolve(providerOrUrl, mockChains);

  // Additional check: if we're dealing with local hardhat addresses, force mock mode
  // This handles cases where MetaMask connects to local network but chainId detection fails
  let isMock = resolvedIsMock;
  let rpcUrl = resolvedRpcUrl;
  if (!isMock && typeof providerOrUrl !== "string") {
    try {
      // Try to detect if we're on a local hardhat network by checking common local addresses
      const testAddress = "0x5c653ca4AeA7F2Da07f0AABf75F85766EAFDA615"; // Known hardhat address
      const code = await providerOrUrl.request({
        method: "eth_getCode",
        params: [testAddress, "latest"]
      });
      // If the address has code, it's likely a deployed contract on local network
      if (code && code !== "0x") {
        console.log("Detected local hardhat network by contract presence, forcing mock mode");
        isMock = true;
      ...
      console.log("Local network detection failed:", e);
    }
  }

  console.log("FHEVM Instance Creation Debug:");
  console.log("- ChainId:", chainId);
  console.log("- IsMock:", isMock);
  console.log("- ResolvedIsMock:", resolvedIsMock);
  console.log("- RpcUrl:", rpcUrl);
  console.log("- Provider type:", typeof providerOrUrl);
  ...
  if (chainId === 11155111) { // Sepolia
    ...
    // For other networks, try to use Sepolia config as fallback
    // This might need to be updated for other testnets
    networkConfig = relayerSDK.SepoliaConfig;
    console.log("Using Sepolia config as fallback for chainId:", chainId);
  }
